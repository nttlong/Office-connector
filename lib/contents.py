...
class DownLoadInfo:
    ...
    def __init__(self, data: dict):
        ...
        self.download_dir = os.path.join(config.app_data_dir)
        ...
        self.file_path = os.path.join(self.download_dir,f"{file_name}")
    ...
    def get_track_file_path(self):
        ...
        ret = os.path.join(lib.config.get_app_track_dir(),self.id)
    ...
    def is_change(self):
        ...
        if self.size_in_bytes != self.get_size_online():
            ...
            if ret:
                return True
            # else:
            #     ret= self.modify_time!=self.get_modify_time_online()
            #     if ret:
            #         return ret
        return False
    ...
    def do_download(self):
        ...
        if os.path.isfile(self.file_path):
            ...
            try:
                ...
                if os.path.isfile(self.get_track_file_path()):
        ...
        if is_existing:
            ...
            if response.status_code == 200:
                ...
                lib.ui_controller.loader.show_message_error(f"Can not download {self.check_out_url}\n"
                                                            ...
                                                            f"Http error description:{response.content.decode('utf8')}")
                lib.loggers.logger.error(response.content.decode("utf8"))
                return False
            return False
    ...
    def do_upload(self):
        import lib.ui_controller
        self.is_ready = False
        lib.ui_controller.loader.show_message(f"Update content to {self.check_in_url} ...")
        count=10
        is_ok = False
        err= None
        while count>0:
            try:
                ...
                err = e
                time.sleep(2)
                lib.ui_controller.loader.show_message(f"Update content to {self.check_in_url} ...({10-count})")
                count-=1

        if not is_ok:
            if isinstance(err,requests.exceptions.HTTPError):
                lib.ui_controller.loader.show_message_error(f"Can not update content to ({self.check_in_url})\n"
                                                            f"Http Error:{err.response.status_code}\n"
                                                            f"Server error description: {err.response.content.decode('utf8')}")
            else:
                lib.ui_controller.loader.show_message_error(f"Can not update content to ({self.check_in_url})")
